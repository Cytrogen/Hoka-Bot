...
__usage__ = f'''game插件可以和群友玩游戏
俄罗斯轮盘：
原作者：【HibiKier】
[装弹] <子弹数> <金额> <at> # 开始对决；<at>为指定目标
[接受] / [拒绝] # 接受对决/拒绝决斗
[开枪] # 开枪
[结算] # 当一方60秒未开枪，可以强行结束对决
[我的战绩] # 我的战绩
[金币排行/胜场排行/败场排行/欧洲人排行/慈善家排行] # 排行榜
【注：同一时间群内只能有一场对决】
...
[卡池] <星数> -> 查看当前卡池角色列表
[我的背包] -> 查看已抽到角色数量
[售卖] <num> 个 <x星> -> 售卖num个x星
    【三星50金币，四星100金币，五星500金币，其余星无法售卖】
【该功能仅在部分群组内授权】
...
sign = on_command('签到', rule=to_me(), permission=GROUP, priority=5, block=True)
@sign.handle()
...
work = on_command('打工', rule=to_me(), permission=GROUP, priority=5, block=True)
@work.handle()
async def _(bot: Bot, event: GroupMessageEvent, state: T_State):
    global player_data
...
@work.got("type", prompt="选择你今天想打的工：\n毁灭世界\n考美院\n当水军")
...
game_rank = on_command('胜场排行', aliases={'金币排行', '胜利排行', '败场排行', '失败排行','欧洲人排行', '慈善家排行'}, permission=GROUP, priority=5, block=True)
@game_rank.handle()
async def _(bot: Bot, event: GroupMessageEvent, state: T_State):
    global player_data
...
my_bag = on_command('我的背包', permission=GROUP, priority=5, block=True)
@my_bag.handle()
async def _(bot: Bot, event: GroupMessageEvent, state: T_State):
    global player_data
    player_is_exists(event)
...
no_gamble = on_keyword("戒赌", rule=to_me(), priority=11)
@no_gamble.handle()
async def ok(bot: Bot, event: GroupMessageEvent, state: T_State):
    await no_gamble.finish("\n" + random.choice([f'你成功戒赌……hoka好欣慰……',
    '你终于发现了hoka的用心良苦了吗……',
    '好戒！开香槟咯！',
    'hoka听到这个消息，好开心……',
    '好！不要做赌狗！']), at_sender=True)
    ...
    for group in player_data.keys():
        for user_id in player_data[group].keys():
            ...
            if player_data[group][user_id]['jail'] >= 1:
                ...
                print('所有刑期日期-1')
            if player_data[group][user_id]['is_study'] >= 1:
