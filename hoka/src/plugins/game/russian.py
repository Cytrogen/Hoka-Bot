...
from nonebot.adapters.cqhttp import GROUP, Bot, MessageEvent, GroupMessageEvent, Message, MessageSegment
from .util import get_message_at, is_number, get_message_text
...
rs_player = {}
rssian = on_command('装弹', aliases={'轮盘'}, permission=GROUP, priority=5, block=True)
@rssian.args_parser
...
accept = on_command('接受', aliases={'接受决斗', '接受挑战', '接受对决'}, permission=GROUP, priority=5, block=True)
@accept.handle()
async def _(bot: Bot, event: GroupMessageEvent, state: T_State):
    global rs_player, player_data
    player_is_exists(event)
...
refuse = on_command('拒绝', aliases={'拒绝决斗', '拒绝挑战', '拒绝对决'}, permission=GROUP, priority=5, block=True)
@refuse.handle()
async def _(bot: Bot, event: GroupMessageEvent, state: T_State):
    global rs_player
    player_is_exists(event)
