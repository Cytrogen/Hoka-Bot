...
__usage__ = '''word_bank插件
【原作者：Joenothing-lst】
#模糊/正则/全局# [问] <触发句> [答] <答句> -> 设置词条(仅 超级用户、群主、管理员)
[删除 #全局# 词条] <触发句> -> 删除特定（或全局）词条（仅 超级用户、群主）
[删除全部词库] （仅 超级用户、群主）'''
__help_plugin_name__ = '轻量问答'
__priority__ = 99
...
async def _(bot: Bot, event: MessageEvent):
    ...
    if msgs:
        if reply_type == 'random':
            ...
            await bot.send(event, message=Message(unescape(parse(msg=msg,
...
wb_set_cmd = on_regex(r"^(?:全局|模糊|正则)*问", permission=SUPERUSER|GROUP_OWNER|GROUP_ADMIN, priority=99)
@wb_set_cmd.handle()
...
wb_del_cmd = on_command('删除词条', permission=SUPERUSER|GROUP_OWNER, priority=99)
@wb_del_cmd.handle()
...
wb_del_admin = on_command('删除全局词条', permission=SUPERUSER|GROUP_OWNER, priority=99)
@wb_del_admin.handle()
...
wb_del_all_cmd = on_command('删除词库', permission=SUPERUSER|GROUP_OWNER, handlers=[wb_del_all], priority=99)
...
wb_del_all_admin = on_command('删除全局词库', permission=SUPERUSER|GROUP_OWNER, handlers=[wb_del_all], priority=99)
...
wb_del_all_bank = on_command('删除全部词库', permission=SUPERUSER|GROUP_OWNER, handlers=[wb_del_all], priority=99)
