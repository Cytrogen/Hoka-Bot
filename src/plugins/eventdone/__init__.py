...
__help_plugin_name__ = '好友申请'
__des__ = '远程好友申请处理'
__author__ = 'PadorFelice'
__level__ = 'N/A'
__cmd__ = '''
N/A
'''.strip()
...
# 超级用户推送添加机器人好友请求事件
add_friend = on_request(friend_request_rule, 
                        priority=1, 
                        block=True)
    ...
    try:
        ...
        for su_qq in super_id:
            await bot.send_private_msg(user_id=int(su_qq),
                                        message=f"QQ：{add_qq} 请求添加Hoka为好友!\n请求添加时间：{realtime}\n验证信息为：{comment}")
    except Exception as e:
        for su_qq in super_id:
            await bot.send_private_msg(user_id=int(su_qq), message=f"Hoka坏掉了\n错误信息：{e}")


# 超级用户使用，同意好友添加机器人请求
agree_qq_add = on_command("同意", 
...
async def _(bot: Bot, event: PrivateMessageEvent):
    try:
        ...
        if agree_id in qq_set:
            await bot.send_private_msg(user_id=user_id, message=f"Hoka成功添加QQ:{agree_id}为好友！")
        ...
        for su_qq in super_id:
            await bot.send_private_msg(user_id=int(su_qq), message=f"Hoka出错了\n错误信息：{e}")