...
jb_player = {}
OWNER = get_driver().config.superusers


work = on_command("找工作", 
                aliases={'职业'}, 
                rule=to_me() & isInService("群职业", 1), 
                priority=5)
@work.handle()
...
async def _(bot: Bot, event: GroupMessageEvent, state: T_State = State()):
    ...
    if '爱豆' in job:
        ...
        user_name = event.sender.card if event.sender.nickname else event.sender.nickname
        await work.send(f"{user_name} 现在是皮条客了！")
        player_data[str(event.group_id)][str(event.user_id)]['is_job'] = True
        player_data[str(event.group_id)][str(event.user_id)]['job'] = 4
    with open(file, 'w', encoding='utf8') as f:
        json.dump(player_data, f, ensure_ascii=False, indent=4)


quitjob = on_command("辞职", 
                    rule=to_me() & isInService("群职业", 1), 
                    priority=5)
@quitjob.handle()
async def _(bot: Bot, event: GroupMessageEvent, state: T_State = State()):
    ...
    if player_data[str(event.group_id)][str(event.user_id)]['is_off'] >= 1:
        await quitjob.finish("你这周已经辞职过了！")
    
    player_data[str(event.group_id)][str(event.user_id)]['is_job'] = False
    player_data[str(event.group_id)][str(event.user_id)]['job'] = 0
    player_data[str(event.group_id)][str(event.user_id)]['is_off'] = 7
    with open(file, 'w', encoding='utf8') as f:
        json.dump(player_data, f, ensure_ascii=False, indent=4)
    await quitjob.finish("你辞职成功，变成无业游民了！")


jobinfo = on_command("职业信息", 
                    aliases={'职业查询'},
                    rule=isInService("群职业", 1),
                    priority=5)
@jobinfo.handle()
async def _(bot: Bot, event: GroupMessageEvent, state: T_State = State(), text: Message = CommandArg()):
    args = text.extract_plain_text()
...
async def _(bot: Bot, event: GroupMessageEvent, state: T_State = State()):
    ...
    lpt = player_data[str(event.group_id)][str(event.user_id)]['is_lpt']
    gold = player_data[str(event.group_id)][str(event.user_id)]['gold']
    off = player_data[str(event.group_id)][str(event.user_id)]['is_off']
...
async def _(bot: Bot, event: GroupMessageEvent, state: T_State = State()):
    ...
    if '签到' in value:
        ...
        player_data[str(event.group_id)][str(event.user_id)]['is_ptt'] = False
    elif '监狱' in value:
        ...
        player_data[str(event.group_id)][str(event.user_id)]['is_study'] = False
    elif '皮条' in value:
        player_data[str(event.group_id)][str(event.user_id)]['is_lpt'] = False
    elif '辞职' in value:
        player_data[str(event.group_id)][str(event.user_id)]['is_off'] = 0
    else:
        await demon.finish('参数不正确')
    with open(file, 'w', encoding='utf8') as f:
        json.dump(player_data, f, ensure_ascii=False, indent=4)
    await demon.finish("修改成功！")


demonall = on_command("alljobfix",
                    rule=isInService("群职业", 1),
                    priority=5)
@demonall.handle()
async def _(bot: Bot, event: GroupMessageEvent, state: T_State = State(), text: Message = CommandArg()):
    ...
    if '-a' in value:
        for group in player_data.keys():
            for user_id in player_data[group].keys():
                ...
                player_data[group][user_id]['is_off'] = 0
    elif '-c' in value:
        for user_id in player_data[str(event.group_id)].keys():
            ...
            player_data[str(event.group_id)][user_id]['is_train'] = False
            player_data[str(event.group_id)][user_id]['is_lpt'] = False
            player_data[str(event.group_id)][user_id]['is_off'] = 0
    elif '-g' in value:
        for group in player_data.keys():
            for user_id in player_data[group].keys():
        ...
        for group in player_data.keys():
            for user_id in player_data[group].keys():
                player_data[group][user_id]['num1'] += 10
    with open(file, 'w', encoding='utf8') as f:
        json.dump(player_data, f, ensure_ascii=False, indent=4)
...
@book.got("type", prompt=f"解读面经需要消耗快乐值，并以【1快乐:100金币】的比例换取金币\n你想要换取多少金币？")
async def _(bot: Bot, event: GroupMessageEvent, state: T_State = State()):
    player_is_exists(event)
...
async def _(bot: Bot, event: GroupMessageEvent, state: T_State = State()):
    ...
    if player_data[str(event.group_id)][str(event.user_id)]['is_jail']:
        if player_data[str(event.group_id)][str(event.user_id)]['jail'] == 0:
            ...
            if player_data[str(event.group_id)][str(event.user_id)]['is_ptt']:
                await potato.finish("你今天削过土豆了！")
    else:
    ...
    await belief.send(dates() + events, at_sender=True)
    player_data[str(event.group_id)][str(event.user_id)]['gold'] += gold
    player_data[str(event.group_id)][str(event.user_id)]['is_ppt'] = True
    with open(file, 'w', encoding='utf8') as f:
        json.dump(player_data, f, ensure_ascii=False, indent=4)
...
async def _(bot: Bot, event: GroupMessageEvent, state: T_State = State()):
    ...
    if player_data[str(event.group_id)][str(event.user_id)]['is_study']:
        await sing.finish("你今天已经自习过了！")
    ...
    player_data[str(event.group_id)][str(event.user_id)]['wisdom'] += brain
    player_data[str(event.group_id)][str(event.user_id)]['is_study'] = True
    with open(file, 'w', encoding='utf8') as f:
        json.dump(player_data, f, ensure_ascii=False, indent=4)
...
#——————————————————————————————————————————————————————————————————————————————————————————#
### 皮条客 ###
pitiao = on_command("拉皮条", 
                    priority=5,
                    rule=isInService("群职业", 1))
@pitiao.handle()
async def _(bot: Bot, event: GroupMessageEvent, state: T_State = State()):
    player_is_exists(event)
    if player_data[str(event.group_id)][str(event.user_id)]['job'] != 4:
        await sing.finish("你不是皮条客，无法拉皮条")
    if player_data[str(event.group_id)][str(event.user_id)]['is_lpt']:
        await sing.finish("今天的拉皮条上限已经到了！")

    rnd = random.Random()
    first_gold = rnd.randint(3000,5000)
    point = rnd.randint(1,3)
    last_gold = rnd.randint(3000,7000)
    last_gold = last_gold * point
    if point == 1:
        comment = '很烂'
    elif point == 2:
        comment = '还可以'
        ...
        comment = '优秀'
    sub_gold = first_gold + last_gold
    loss_gold = math.floor(sub_gold * 0.25)
    gold = sub_gold - loss_gold
    events = random.choice(['今天的顾客觉得你的鸨儿 ' + comment + f' ，支付了你 {last_gold} 金币\n加上订金 {first_gold}，一共是 {sub_gold} 金币\n鸨儿获得了 {loss_gold} 金币，你获得了 {gold} 金币',
                            '顾客表示你的鸨儿 ' + comment + f' 并给了你 {last_gold} 金币\n加上订金 {first_gold}，一共是 {sub_gold} 金币\n鸨儿获得了 {loss_gold} 金币，你获得了 {gold} 金币',
                            '因为你的鸨儿 ' + comment + f' ，所以顾客支付了你 {last_gold} 金币\n加上订金 {first_gold}，一共是 {sub_gold} 金币\n鸨儿获得了 {loss_gold} 金币，你获得了 {gold} 金币'])
    player_data[str(event.group_id)][str(event.user_id)]['gold'] += gold
    player_data[str(event.group_id)][str(event.user_id)]['make_gold'] += gold
    await pitiao.send(dates() + events, at_sender=True)
    player_data[str(event.group_id)][str(event.user_id)]['is_lpt'] = True
    with open(file, 'w', encoding='utf8') as f:
        json.dump(player_data, f, ensure_ascii=False, indent=4)